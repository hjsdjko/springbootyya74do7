<template>
	<div class="addEdit-block" :style='{"padding":"30px","fontSize":"16px","color":"#666","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="停车场名称" prop="tingchechangmingcheng">
					<el-input v-model="ruleForm.tingchechangmingcheng" placeholder="停车场名称" clearable  :readonly="ro.tingchechangmingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="停车场名称" prop="tingchechangmingcheng">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="小时/元" prop="shoufeibiaozhun">
					<el-input-number v-model="ruleForm.shoufeibiaozhun" placeholder="小时/元" :readonly="ro.shoufeibiaozhun"></el-input-number>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="小时/元" prop="shoufeibiaozhun">
					<el-input v-model="ruleForm.shoufeibiaozhun" placeholder="小时/元" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="出场状态" prop="chuchangzhuangtai">
					<el-select :disabled="ro.chuchangzhuangtai" v-model="ruleForm.chuchangzhuangtai" placeholder="请选择出场状态" >
						<el-option
							v-for="(item,index) in chuchangzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="出场状态" prop="chuchangzhuangtai">
					<el-input v-model="ruleForm.chuchangzhuangtai"
						placeholder="出场状态" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-if="type!='info' && !ro.chepaizhaopian" label="车牌照片" prop="chepaizhaopian">
					<file-upload
						tip="点击上传车牌照片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.chepaizhaopian?ruleForm.chepaizhaopian:''"
						@change="chepaizhaopianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="upload" v-else-if="ruleForm.chepaizhaopian" label="车牌照片" prop="chepaizhaopian">
					<img v-if="ruleForm.chepaizhaopian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.chepaizhaopian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.chepaizhaopian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
			</template>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='chepaizhaopian'){
							this.ruleForm.chepaizhaopian = obj[o];
				...
				this.ruleForm.chuchangzhuangtai = '待出场'; 
	...
	if(this.ruleForm.chepaizhaopian!=null) {
		this.ruleForm.chepaizhaopian = this.ruleForm.chepaizhaopian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    chepaizhaopianUploadChange(fileUrls) {
	    this.ruleForm.chepaizhaopian = fileUrls;
        this.$http({
            url: "cheliangruchang/baidu/ocr?fileName="+fileUrls.substr(fileUrls.lastIndexOf("/")+1), 
            method: "get", 
            params: {} 
        }).then(({ 
            data 
