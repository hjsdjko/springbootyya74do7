		...
		<view :style='{"padding":"20rpx  0  120rpx 0px","background":"#162A68","flexDirection":"column","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			...
			<view :style='{"padding":"0 20rpx","margin":"20rpx 0","flexWrap":"wrap","background":"#162A68","display":"flex","position":"relative","height":"auto"}'>
				<scroll-view scroll-y="true" class="category-two" :style='{"width":"22%","margin":"0 30rpx 0 0","height":"100%"}'>
					<view :class='categoryName === item.cheweileixing ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.cheweileixing)">{{item.cheweileixing}}</view>
				</scroll-view>
			<!-- 样式1 -->
			<view class="uni-product-list" :style='{"padding":"120rpx 20rpx  0 20rpx","margin":"0 ","borderRadius":"16rpx 16rpx","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240110/32684fc98f5f4a79a7b03f8dc479f388.png)  no-repeat  top / 100% auto,rgba(225,225,225,0.5) ","flex":"1","display":"flex","width":"73%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"margin":"0 0 20rpx","backgroundColor":"#fff","borderRadius":"20rpx","flexWrap":"wrap","background":"linear-gradient(to bottom, #2DF1FD, #2148D9)","display":"flex","width":"48%","overflow-x":"hidden","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#FFFFFF","width":"100%","lineHeight":"1.4","fontSize":"32rpx","fontWeight":"400","order":"1"}'>区域:{{product.quyu}}</view>
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#FFFFFF","width":"100%","lineHeight":"1.4","fontSize":"32rpx","fontWeight":"400","order":"1"}'>{{product.cheweileixing}}</view>
					...
					<view :style='{"padding":"0 0 0 10rpx","background":"#fff","flex":"1","order":"5"}'>
						<text class="icon iconfont icon-shoucang10" :style='{"margin":"0","lineHeight":"1.5","fontSize":"24rpx","color":"#666"}'></text>
						<text :style='{"color":"#666","lineHeight":"1.5","fontSize":"24rpx"}'>{{product.storeupnum}}</text>
					</view>
					<view :style='{"padding":"0 0 0 4rpx","background":"#fff","flex":"1","order":"5"}'>
					...
					<view :style='{"padding":" 10rpx","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tingchechang','修改')) || (!userid && isAuthFront('tingchechang','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tingchechang','删除')) || (!userid && isAuthFront('tingchechang','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"padding":" 10rpx","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tingchechang','修改')) || (!userid && isAuthFront('tingchechang','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('tingchechang','删除')) || (!userid && isAuthFront('tingchechang','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('cheweileixing', {page:1,limit:100});
            } else {
                res = await this.$api.list('cheweileixing', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,cheweileixing:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.cheweileixing = '%' + this.categoryName + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`tingchechang`, params);
                } else {
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`tingchechang`, searchForm);
                } else {
                    res = await this.$api.list(`tingchechang`, searchForm);
                }
				// 如果是第一页数据置空
