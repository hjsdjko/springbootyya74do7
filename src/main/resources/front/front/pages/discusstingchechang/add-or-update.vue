	...
	<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 120rpx 0","background":" #162A68","height":"auto"}'>
		<form :style='{"padding":"40rpx 20rpx","margin":"0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"rgba(255,255,255,0.3)","display":"flex","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0","flex":"1","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#fff","fontWeight":"500"}' class="title">评论内容</view>
                <xia-editor ref="editor" :style='{"minHeight":"300rpx","border":"0","padding":"20rpx","borderRadius":"8rpx","color":"#666","background":"#141332","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusstingchechang`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusstingchechang`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusstingchechang`, this.ruleForm);
						}else{
							await this.$api.add(`discusstingchechang`, this.ruleForm);
						}
						uni.setStorageSync('discusstingchechangCleanType',true);
						var res1 = await this.$api.info('tingchechang',this.ruleForm.refid)
						res1.data.discussnum++
						await this.$api.update('tingchechang',res1.data)
