...
<view class="content">
	<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 120rpx 0","background":" #162A68","height":"auto"}'>
		<form :style='{"padding":"40rpx 20rpx","margin":"0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"rgba(255,255,255,0.3)","display":"flex","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"0","borderColor":"#ccc","margin":"20rpx auto 20rpx","alignItems":"center","borderWidth":"0","background":"none","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="" @tap="imageTap">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#fff","textAlign":"left"}' class="title">image</view>
				<image :style='{"width":"50rpx","borderRadius":"100%","objectFit":"cover","display":"block","height":"50rpx"}' class="avator" v-if="ruleForm.image" :src="baseUrl+ruleForm.image.split(',')[0]" mode="aspectFill"></image>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`cheweileixing`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`cheweileixing`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`cheweileixing`, this.ruleForm);
						}else{
							await this.$api.add(`cheweileixing`, this.ruleForm);
						}
