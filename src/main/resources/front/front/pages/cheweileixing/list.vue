		...
		<view :style='{"padding":"20rpx  0  120rpx 0px","background":"#162A68","flexDirection":"column","display":"flex","width":"100%","position":"relative","height":"auto"}'>
			...
			<view class="uni-product-list" :style='{"padding":"120rpx 20rpx  0 20rpx","margin":"0 ","borderRadius":"16rpx 16rpx","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240110/32684fc98f5f4a79a7b03f8dc479f388.png)  no-repeat  top / 100% auto,rgba(225,225,225,0.5) ","flex":"1","display":"flex","width":"73%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"margin":"0 0 20rpx","backgroundColor":"#fff","borderRadius":"20rpx","flexWrap":"wrap","background":"linear-gradient(to bottom, #2DF1FD, #2148D9)","display":"flex","width":"48%","overflow-x":"hidden","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<image :style='{"padding":"0","margin":"20rpx","objectFit":"cover","borderRadius":"20rpx","display":"block","width":"calc(100% - 40rpx)","height":"260rpx","order":"2"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.image)" :src="product.image.split(',')[0]"></image>
					<image :style='{"padding":"0","margin":"20rpx","objectFit":"cover","borderRadius":"20rpx","display":"block","width":"calc(100% - 40rpx)","height":"260rpx","order":"2"}' mode="aspectFill" class="uni-product-image" v-else :src="product.image?baseUrl+product.image.split(',')[0]:''"></image>
					...
					<view :style='{"padding":" 10rpx","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheweileixing','修改')) || (!userid && isAuthFront('cheweileixing','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheweileixing','删除')) || (!userid && isAuthFront('cheweileixing','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"padding":" 10rpx","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"11"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheweileixing','修改')) || (!userid && isAuthFront('cheweileixing','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('cheweileixing','删除')) || (!userid && isAuthFront('cheweileixing','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"20rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`cheweileixing`, params);
                } else {
                    res = await this.$api.list(`cheweileixing`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('cheweileixing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.search()
						}
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`cheweileixing`, searchForm);
                } else {
                    res = await this.$api.list(`cheweileixing`, searchForm);
                }
				// 如果是第一页数据置空
